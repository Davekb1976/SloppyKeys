"""Tests for LobbyNavigator.find_and_select_golden_hour_stage and close_stage_list."""
...
try:
    ok, msg = nav.find_and_select_golden_hour_stage(max_scrolls=3, notches=4)
    assert ok is False
...
try:
    mock_match = ImageMatch(
        profile_name="golden_hour", score=0.92, center_x=300, center_y=400, left=275, top=390, width=50, height=20
    )
    ...
    ok, msg = nav.find_and_select_golden_hour_stage(max_scrolls=3, notches=4)
    assert ok is True
    assert "selected Golden Hour stage" in msg
...
# 5. close_stage_list test
nav._engine = MagicMock()
nav._engine.template_exists.return_value = True
nav._find_click = MagicMock(return_value=(True, "closed"))

ok, msg = nav.close_stage_list()
assert ok is True
assert msg == "closed"
assert nav._find_click.called

nav._engine.template_exists.return_value = False
ok, msg = nav.close_stage_list()
assert ok is False
assert "missing" in msg

print("Golden Hour scanner test: OK")
