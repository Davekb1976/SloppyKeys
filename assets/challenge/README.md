...
**The close button is not here.** Leaving the list uses the same X as every other panel in
the gamemode UI, so it is one shared file — `assets/lobby/close.png` — rather than a
challenge-specific crop of the same pixels. And closing the list is only half of leaving: the
chooser underneath it needs `lobby/close_gamemode.png` after it, or the next task starts
against a covered screen. Both are documented in `assets/lobby/README.md`.
...
Nothing else belongs here. There are deliberately **no** map-name templates: five maps times
seven acts would be 35 crops, all of them stale the next patch.
